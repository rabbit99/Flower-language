...
namespace Gamekit2D
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        //public void OnCheckInventoryItem(Damager damager, Damageable damageable)
        //{
        //    switch (damager.name)
        //    {
        //        case "FireWall":
        ...
        {
            Debug.Log("use the item named " + itemName);
            StartCoroutine(_useItem(itemName));
        }

        public bool GetHurtResult(Damager damager)
        {
            switch (damager.name)
            {
                case "FireWall":
        ...
        {
            if (m_InventoryController.HasItem(itemName)){
                switch (itemName)
                {
                    case "cactus":
                        onCactusBuff = true;
                ...
                Debug.Log("start buff named " + itemName);
                yield return new WaitForSeconds(ItemLookUp.ItemLookUpTable[itemName]);
                switch (itemName)
                {
                    case "cactus":
                        onCactusBuff = false;
